...
static const int64_t OLD = INT64_MAX; /* gross */
...
typedef struct munkee munkee;
typedef struct munkee {
  int64_t inspection;
  int64_t div_by;
  expr expr;
  munkee * T;
...
static void read_op(const char ** s, munkee * M);
static void read_test(const char ** s, munkee * M);
static void read_if_true(const char ** s, munkee * M);
static void read_if_false(const char ** s, munkee * M);
static void read_munkee(const char ** s);
static int int_cmp(const void * l, const void * r);

static size_t input_len = 0;
static char * input = NULL;
static size_t munkees_len = 0;
static munkee munkees[64];

static void run_round(bool div_by_3) {
  /* lcm only needed by puzzle 1: */
  int64_t lcm = munkees[0].div_by;
  for(size_t i = 1; i < munkees_len; i++) {
    lcm *= munkees[i].div_by;
  }

  for(size_t i = 0; i < munkees_len; i++) {
    munkee * M = &munkees[i];
    size_t max = M->items_len;
    for(size_t j = 0; j < max; j++) {
      M->inspection++;

      int64_t * lhs = &M->items[j],
                rhs = M->expr.rhs;
      if(M->expr.rhs == OLD) { rhs = *lhs; }
      ...
      *lhs = M->expr.op == ADD
      ...
      if(div_by_3) {
        /* Puzzle 0 */
        int64_t new_worry = (int64_t)floor((double)*lhs / (double)3);
        munkee * to = new_worry % M->div_by == 0 ? M->T : M->F;
        to->items[to->items_len++] = new_worry;
      } else {
        /* Puzzle 1 (I needed a hint for the LCM for this :( */
        *lhs %= lcm;
        munkee * to = *lhs % M->div_by == 0 ? M->T : M->F;
        to->items[to->items_len++] = *lhs;
      }
      M->items_len--;
    }
  }
...
static int int_cmp(const void * l, const void * r) {
  return (int)(int64_t)(*(const int64_t *)l - *(const int64_t *)r);
}
...
*/
int aoc_day10_p0(int argc, char **argv) {
  static const bool div_by_3 = true;
  int result = read_lines(argc, argv, NULL, &read_script);
  if(!input || input_len == 0) { return EXIT_FAILURE; }

  const char * s = input;
  while(s < s + input_len && *s != '\0') {
    read_munkee(&s);
  }
  ...
  int64_t mb[64] = { 0 };
  for(size_t i = 0; i < munkees_len; i++) {
    mb[mb_len++] = munkees[i].inspection;
  }
  qsort(mb, mb_len, sizeof * mb, &int_cmp);

  fprintf(stdout, "Monkey Business (%" PRId64 " * %" PRId64 "): %" PRId64 "\n", mb[mb_len - 2], mb[mb_len - 1], mb[mb_len - 2] * mb[mb_len - 1]);
...
int aoc_day10_p1(int argc, char **argv) {
  static const bool div_by_3 = false;
  int result = read_lines(argc, argv, NULL, &read_script);
  if(!input || input_len == 0) { return EXIT_FAILURE; }
  ...
  fprintf(stdout, "Monkey Business 10K (%" PRId64 " * %" PRId64 "): %" PRId64 "\n", mb[mb_len - 2], mb[mb_len - 1], mb[mb_len - 2] * mb[mb_len - 1]);
  free(input), input = NULL;
  return result;
}
...
static void read_starting(const char ** s, munkee * M) {
  static const size_t StartingItems = sizeof "  Starting items: ";
  *s += StartingItems;  /* "  Starting items: " */
  read_list(s, M);
}

static inline op parse_op(const char ** s) {
  const char o = **s;
  (*s)++;
  switch(o) {
    case '+': return ADD;
...
static int64_t parse_expr(const char ** s) {
  static const size_t Old = sizeof "old";
  int64_t e = 0;
  if(**s == 'o') {
    *s += Old; (*s)--;
    e = OLD;
...
static void read_op(const char ** s, munkee * M) {
  static const size_t Operation = sizeof "  Operation: new = ";

  *s += Operation;
  ws(s); M->expr.lhs = parse_expr(s);
  ws(s); M->expr.op = parse_op(s);
  ws(s); M->expr.rhs = parse_expr(s);
}

static void read_test(const char ** s, munkee * M) {
  static const size_t Test = sizeof "  Test: divisible by ";
  (*s) += Test;
  M->div_by = read_int(s);
}

static void read_if_true(const char ** s, munkee * M) {
  static const size_t IfTrue = sizeof "    If true: throw to monkey ";
  (*s) += IfTrue;
  M->T = &munkees[read_int(s)];
}

static void read_if_false(const char ** s, munkee * M) {
  static const size_t IfFalse = sizeof "    If false: throw to monkey ";
  (*s) += IfFalse;
  M->F = &munkees[read_int(s)];
}

static void read_munkee(const char ** s) {
  static const size_t Monkey = sizeof "Monkey";
  munkee * M = &munkees[munkees_len++];
  *s += Monkey; /* "Monkey " */
  read_int(s); /* throw away value */
  read_starting(s, M);
  read_op(s, M);
  read_test(s, M);
  read_if_true(s, M);
...
static void read_list(const char **s, munkee * M) {
  ...
  do {
    ...
    while(true) {
      if(**s == ',' || **s == '\n') {
        const char * t = buf;
        M->items[M->items_len++] = read_int(&t);
        if(**s == ' ') (*s)++;
        if(**s == ',') (*s)++;
        if(**s == '\n') done = true;
        break;
